...
# Optional values
COLLECTOR_PACKAGE_REPO_HOST="tanzu-platform.packages.broadcom.com"
K8S_COLLECTOR_DEFAULT_HELM_REPO="oci://${COLLECTOR_PACKAGE_REPO_HOST}/adie/helm-charts/k8s-collector"
PXLSCRIPTS_DEFAULT_HELM_REPO="oci://${COLLECTOR_PACKAGE_REPO_HOST}/adie/helm-charts/pxl-scripts"
TELEGRAF_DEFAULT_HELM_REPO="oci://${COLLECTOR_PACKAGE_REPO_HOST}/adie/helm-charts/telegraf-collector"
...
VMW_COLLECTOR_VERSION_OVERRIDE="3.24.0"
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      ...
      --package-repository-host)
        COLLECTOR_PACKAGE_REPO_HOST="$2"
        K8S_COLLECTOR_HELM_REPO="oci://${COLLECTOR_PACKAGE_REPO_HOST}/adie/helm-charts/k8s-collector"
        PXLSCRIPTS_DEFAULT_HELM_REPO="oci://${COLLECTOR_PACKAGE_REPO_HOST}/adie/helm-charts/pxl-scripts"
        TELEGRAF_HELM_REPO="oci://${COLLECTOR_PACKAGE_REPO_HOST}/adie/helm-charts/telegraf-collector"
...
collector:
  ...
  image:
    repository: "${COLLECTOR_PACKAGE_REPO_HOST}/aria/k8s-collector"
EOF
  echo "${temp_file}"
}
...
telegraf:
  ...
  image:
    repository: "${COLLECTOR_PACKAGE_REPO_HOST}/telegraf"
EOF
  echo "${temp_file}"
}
...
install_telegraf() {
  ...
  if [[ $VMW_INSTALL_TELEGRAF -eq 1 ]]; then
    ...
    then
      ...
      run_command ${USABLE_HELM} install --insecure-skip-tls-verify aria-telegraf-collector "${TELEGRAF_HELM_REPO}" \
        --create-namespace \
        ${HELM_DEBUG_OPT} \
    ...
    # Upgrade the chart
    # shellcheck disable=SC2086
    run_command ${USABLE_HELM} upgrade --insecure-skip-tls-verify \
...
install_k8s_collector() {
  ...
  if [[ $VMW_INSTALL_K8S -eq 1 ]]; then
    ...
    then
      ...
      # shellcheck disable=SC2086
      run_command ${USABLE_HELM} install --insecure-skip-tls-verify \
        aria-k8s-collector \
